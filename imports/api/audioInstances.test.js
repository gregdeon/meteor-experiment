...
describe('processResults', function() {
    it('computes diffs and results for a finished task', function() {
        ...
        let audio_task = {
            ...
            words_p1: normalizeWord('So you were, for many years, obsessed with the idea of bringing the Olympics, the Summer Olympics, to New York City'),
            words_p2: normalizeWord('So you were obcessed with briging the Olympics, the Summer Olympics, to New York City. And you toiled for a while, spending money, recruiting people to your cause. You got levrage over time, you got in the administration'),
            reward_mode: 1,
        }

        let audio_instance = {
            words_typed: normalizeWord('you, for many years, obsessed with the idea of bringing the Summer Olympics to New York. And you toiled for a while, spending a lot, recruiting a lot to your cause. You got leverage over time. You were reluctant to take that job,'),
        }

        let results = processResults(audio_task, audio_instance)

        // Check diffs
        expect(results.diffs[0]).to.deep.equal(diffWords(audio_task.words_truth, audio_task.words_p1))
        expect(results.diffs[1]).to.deep.equal(diffWords(audio_task.words_truth, audio_task.words_p2))
        expect(results.diffs[2]).to.deep.equal(diffWords(audio_task.words_truth, audio_instance.words_typed))
